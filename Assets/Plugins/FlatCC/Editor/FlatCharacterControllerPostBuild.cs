...
public class FlatCharacterControllerPostBuild
{
            ...
            if (reader != null)
            {
                ...
                using (StreamWriter writer = WaitForFileWrite(pathToVersionsFile))
                {
                    if (writer != null)
                    {
                        ...
                        WriteOrUpdateVersion(allLines, writer, CharacterControllerEditorWindow.pluginName, CharacterControllerEditorWindow.versionNumber);
                        writer.Close();
                    }
                    else
                    {
                        Debug.Log("Unable to write " + CharacterControllerEditorWindow.pluginName + " version number to file.");
                    }
                }
            }
            else
