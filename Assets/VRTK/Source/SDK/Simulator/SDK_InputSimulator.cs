namespace VRTK
{
    ...
    public class SDK_InputSimulator : MonoBehaviour
    {
        ...
        //commenting some of these due to NREs for the desktop ver - FigJ
        ...
        public float timeDiv = 0f; //to more easily adjust player speed in the editor
        ...
        //alternative movement for desktop exe - FigJ
        [Header("Alt Movement Key Binding Settings")]

        [Tooltip("Alternate key used to move forward.")]
        public KeyCode moveForwardAlt = KeyCode.UpArrow;
        [Tooltip("Alternate key used to move to the left.")]
        public KeyCode moveLeftAlt = KeyCode.LeftArrow;
        [Tooltip("Alternate key used to move backwards.")]
        public KeyCode moveBackwardAlt = KeyCode.DownArrow;
        [Tooltip("Alternate key used to move to the right.")]
        public KeyCode moveRightAlt = KeyCode.RightArrow;
        [Tooltip("Alternate key used to sprint.")]
        public KeyCode sprintAlt = KeyCode.RightShift;
       ...
       /* protected virtual void TryPickup(bool rightHand)
        ...
        {
            Vector3 mouseDiff = GetMouseDelta();
            //to allow for up/down movement when using the mouse to look around

            if (IsAcceptingMouseInput())
            {
                Vector3 rot = transform.localRotation.eulerAngles;
                rot.y += (mouseDiff * playerRotationMultiplier).x / 3f;
                transform.localRotation = Quaternion.Euler(rot);

                //the camera needs to be dependent on the mouse, not the neck of the player
                ...
                {
                    rot.x += ((mouseDiff * playerRotationMultiplier).y * -1) / 3f;
        ...
        protected virtual void UpdatePosition()
        {
            ...
            Transform tempValue = this.transform;
            ...
            if (Input.GetKey(moveForward) || Input.GetKey(moveForwardAlt))
            {
                forwardMotion = true;
                if (moveMod < maxSpd)
                    moveMod = Mathf.Lerp(moveMod, maxSpd,
                        Time.deltaTime * playerMoveMultiplier);
                else moveMod = maxSpd;
                tempValue.Translate(transform.forward * moveMod, Space.World);
            }
            ...
            else if (!Input.GetKey(moveLeft) && !Input.GetKey(moveLeftAlt) &&
                     !Input.GetKey(moveRight) && !Input.GetKey(moveRightAlt))
            {
                moveMod = minSpd;
            }
            ...
            if (Input.GetKey(moveLeft) || Input.GetKey(moveLeftAlt))
            {
                ...
                tempValue.Translate(-transform.right * moveMod, Space.World);
            }
            else if (Input.GetKey(moveRight) || Input.GetKey(moveRightAlt))
            {
                if (!forwardMotion)
                    if (moveMod < maxSpd)
                        moveMod = Mathf.Lerp(moveMod, maxSpd,
            ...
            transform.position = new Vector3(tempValue.position.x, baseY,tempValue.position.z);
        }
