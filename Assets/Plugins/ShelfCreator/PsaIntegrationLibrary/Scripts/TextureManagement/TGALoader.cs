...
public static class TGALoader
{
    public static Texture2D LoadTGA(string fileName)
    {
        ...
        {
            Texture2D newTex = LoadTGA(imageFile);
            // Change the texture's name to match its image file name
            string textureName;
            // If it's a .# file, keep the extension
            if (ApplyImagesToProducts.IsDotNumberExtension(Path.GetExtension(fileName)))
            {
        ...
        using (BinaryReader r = new BinaryReader(TGAStream))
        {
            ...
            if (bitDepth == 32)
            {
                for (int i = 0; i < width * height; i++)
                {
                    ...
                    pulledColors[i] = new Color32(blue, green, red, (byte)~alpha);
                }
            }
            else if (bitDepth == 24)
            {
